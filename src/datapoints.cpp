...
void load_data_from_file(const char* fname, int& N, value_type** x, value_type** y)
{
	...
	//			(determined by file, not given to function as input)
	...
	// determine n
	if(static_cast<std::string>(fname).find("blob")!=std::string::npos){
		// if it's one of those weird TA files
		char dummy[100];
...
// the random values will be in an interval of length l, centered around 0
void load_data_random(int N, double l, value_type** x, value_type** y) {
	...
	for (unsigned int i(0); i < N; i++) {
		(*x)[i] = drand48()*l - shift ; //std::cout << "x " << i << "   " << (*x)[i] << std::endl;
		(*y)[i] = drand48()*l - shift ; //std::cout << "y " << i << "   " << (*y)[i] << std::endl;
	}
}
...
void load_data_random(int N, double l, value_type** q)
{
	...
	// l 		length of the interval in which we want to have data generated
	// OUTPUT
...
void clean_file(const char* fname){
	...
	// if a file with this name already exists, its contents are discarded and the file is treated as a new empty file.
	fclose(f);
}
...
void write_to_file(const char* fname, int N, const value_type* x, const value_type* y){
    ...
    // write array content
    fwrite(x, sizeof(value_type), N, f);		// write the array contents in binary format
    fwrite(y, sizeof(value_type), N, f);		// write the array contents in binary format
